...
type ServiceNode struct {
	...
	retireNoServiceFunc func(context.Context) bool
}

func NewOneClusterServiceNode(store MetaStore, ddlID string, retireNoServiceFunc func(context.Context) bool) *ServiceNode {
	...
	return &ServiceNode{
		...
		store:               store,
		cancel:              cancel,
		retireNoServiceFunc: retireNoServiceFunc,
	}
}
...
func (n *ServiceNode) coordinateLoop(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			if ticks >= nextTick {
				retired := n.retireNoServiceFunc(ctx)
				if retired {
					nextTick += 1
				} else {
					nextTick += 10
			...
			ticks++
		case <-ctx.Done():
			logutil.BgLogger().Info("coordinateLoop will quit because context done")
			return
		}
	}
}
